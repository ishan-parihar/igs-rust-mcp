...
pub async fn env_epa_facilities(input: EnvEpaFacilitiesInput) -> Result<EnvEpaFacilitiesOutput, String> {
...
pub async fn env_epa_emissions(input: EnvEpaEmissionsInput) -> Result<EnvEpaEmissionsOutput, String> {
    let settings = config::load_settings().await.map_err(|e| format!("Settings: {}", e))?;
    let cache_dir = http_mod::resolve_cache_dir(&settings, &config::user_config_dir());
    let http = HttpClient::new(&settings.http, &cache_dir);
